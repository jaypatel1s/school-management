  ...
  module V1
    module Teachers
      class ExamsController < BaseController
        ...
        def index
          exams = current_college.exams.joins(:course).where(courses: { teacher_id: current_user.teacher.id }).includes(:course, :results).where(@conditions)
        ...
        def set_exam
          @exam = current_college.exams.joins(:course).where(courses: { teacher_id: current_user.teacher.id }).find(params[:id])
