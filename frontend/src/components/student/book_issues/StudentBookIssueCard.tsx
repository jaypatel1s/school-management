import { StudentBookIssue } from '@/types/students';
import Link from 'next/link';

interface StudentBookIssueCardProps {
  issue: StudentBookIssue;
}

export function StudentBookIssueCard({ issue }: StudentBookIssueCardProps) {
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 text-sm">
        ...
        {issue.return_date && (
          <div className="flex justify-between">
            ...
            <span className="text-gray-900">{issue.return_date}</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 pt-4 border-t">
        <Link 
          href={`/student/book_issues/${issue.id}`}
